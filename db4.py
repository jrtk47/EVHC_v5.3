...
TRANSLATIONS = {
    ...
    "hdr_title":         {"EN": "Static Thermal Rating-based EV Hosting Capacity Predictor"},
    ...
    "fast_charger_cap":  {"EN": "11 kW fast charger: **{w}%**", "TH": "11 kW fast charger: **{w}%**"},
    ...
    "section_jump_analysis":     {"EN": "Jump to analysis", "TH": "ไปยังการวิเคราะห์"},
    ...
    "analyze_select":    {"EN": "Select a transformer to analyze", "TH": "เลือกหม้อแปลงที่ต้องการวิเคราะห์"},
    "analyze_btn":       {"EN": "Analyze →", "TH": "วิเคราะห์ →"},
...
tab_fleet, tab_analysis = st.tabs([t("tab_fleet", lang), t("tab_analysis", lang)])
...
with tab_analysis:
    ...
    with st.sidebar:
        ...
        ref_upload = st.file_uploader(t("sb1_uploader", lang), type=["csv"], key="_ratings_ref_upload")
        if ref_upload is not None and st.session_state.get("_last_ref_upload_name") != ref_upload.name:
            ...
            try:
                ...
                st.success(t("ratings_loaded", lang, n=len(parsed)))
...
with tab_fleet:
    ...
    if not fleet_valid.empty:
        ...
        st.markdown(f'<div class="section-lbl">{t("section_jump_analysis", lang)}</div>', unsafe_allow_html=True)
        pick_col, btn_col = st.columns([3, 1])
        with pick_col:
            pick_name = st.selectbox(t("analyze_select", lang), fleet_valid["Name"].tolist(), label_visibility="collapsed")
        with btn_col:
            if st.button(t("analyze_btn", lang), width='stretch'):
