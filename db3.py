...
CHONBURI_LAT, CHONBURI_LON = 13.3611, 100.9847
...
def parse_ratings_reference(file_bytes: bytes) -> dict:
    ...
    for _, r in df.iterrows():
        ...
        if name_val and name_val.lower() != 'nan':
            ratings[name_val] = float(r[kva_col])
        if other_col:
            other_val = str(r[other_col]).strip()
            if other_val and other_val.lower() != 'nan':
                ratings[other_val] = float(r[kva_col])
    return ratings, name_col, kva_col
...
with st.sidebar:
    st.toggle("Show explanations", value=True, key="show_explanations")
    st.markdown("---")

st.markdown("""
<div class="hdr">
  <div class="hdr-eyebrow">PEA กฟก.2 · Chonburi Grid</div>
  <div class="hdr-title">EV Hosting Capacity Predictor</div>
  <div class="hdr-sub">Hybrid Ensemble ML Engine (RidgeCV + XGBoost Residuals) · LOGO-CV Validated</div>
</div>
""", unsafe_allow_html=True)

tab_fleet, tab_analysis = st.tabs(["🗺️  Fleet Overview", "🔍  Transformer Analysis"])

# ══════════════════════════════════════════════════════════════
# TAB 2 — TRANSFORMER ANALYSIS
# ══════════════════════════════════════════════════════════════
with tab_analysis:
    ...
    with st.sidebar:
        ...
        st.markdown('<div class="sb-lbl">1 · Ratings Master CSV (Tier 1)</div>', unsafe_allow_html=True)
        if st.session_state.show_explanations:
            st.caption("Upload once per session: a CSV with PEA No / DTMS No and their kVA rating. Takes priority over the fallback selectbox below.")
        ref_upload = st.file_uploader("Upload ratings master CSV", type=["csv"], key="_ratings_ref_upload")
        if ref_upload is not None and st.session_state.get("_last_ref_upload_name") != ref_upload.name:
            ...
            try:
                parsed, name_col, kva_col = parse_ratings_reference(ref_upload.getvalue())
                st.session_state.setdefault("ratings_reference", {})
                st.session_state.ratings_reference.update(parsed)
                st.success(f"Loaded {len(parsed)} ratings.")
            except Exception as e:
                st.error(f"Could not read reference file: {e}")
        n_ref = len(st.session_state.get("ratings_reference", {}))
        if n_ref:
            st.caption(f"📋 {n_ref} rating(s) loaded this session.")

        st.markdown('<div class="sb-lbl">2 · Upload Load Curve(s)</div>', unsafe_allow_html=True)
        upload_mode = st.radio("Mode", ["Single transformer", "Batch (multiple files)"], horizontal=True, label_visibility="collapsed")
        shift_utc = st.checkbox("Timestamps are UTC — convert to UTC+7", value=True)

        uploaded = None
        batch_files = None
        if upload_mode == "Single transformer":
            uploaded = st.file_uploader("Upload DTMS transformer CSV", type=["csv"])
        else:
            batch_files = st.file_uploader(
                "Upload multiple DTMS CSVs", type=["csv"], accept_multiple_files=True, key="_batch_uploader"
            )
        ...
        st.markdown('<div class="sb-lbl">3 · Transformer Rating — Sidebar Fallback (Tier 2)</div>', unsafe_allow_html=True)
        ...
        tr_rated = st.selectbox(
            "Nameplate kVA (used only if no reference match)",
        ...
        if pending_match_note:
            ...
            if st.session_state.show_explanations:
                if kind == "match":
                    st.caption(f"**{pea_id}**: {val:.0f} kVA (from {matched_by})")
                else:
                    st.caption(f"**{pea_id}**: no reference match — using {val:.0f} kVA.")

        st.markdown('<div class="sb-lbl">4 · Day Type</div>', unsafe_allow_html=True)
        day_type   = st.radio("Profile to evaluate", ["Weekday", "Weekend / Holiday"], horizontal=True)
        is_weekend = 1 if "Weekend" in day_type else 0

        st.markdown('<div class="sb-lbl">5 · EV Charging Start Time</div>', unsafe_allow_html=True)
        peak_label = st.selectbox("Peak charging hour", list(PEAK_HOURS.keys()), index=2)
        peak_hour  = PEAK_HOURS[peak_label]

        st.markdown('<div class="sb-lbl">6 · Charger Mix (% share per tier)</div>', unsafe_allow_html=True)
        ...
        st.caption(f"11 kW fast charger: **{w_fast}%**")
        
        total_w = w_slow + w_std + w_fast
        if total_w == 0:
            st.error("Weights must not all be zero.")
            st.stop()
            ...
            for i, f in enumerate(batch_files):
                ...
                try:
                    ...
                    if {'timestamp', 'P_TOT', 'S_TOT'} - _probe_cols:
                        ...
                        upsert_fleet_result(fid, rated_b, None, None, hc_adj_b, status_b, verified=bool(matched_by_b))
    ...
    if cached_names:
        ...
        inspect_pick = st.selectbox(
            "📂 Inspect uploaded transformer", cached_names, index=default_idx, key="_inspect_pick"
        )
    ...
    if active_bytes is None:
        st.markdown(
            ...
            '<div class="empty-title">Upload a transformer load curve to begin</div></div>',
            unsafe_allow_html=True
        )
    else:
        missing = None
        ...
        if missing:
            ...
            if feats['Capacity_Headroom_kVA'] <= 0:
                st.warning(f"Peak apparent power ({feats['_peak_kva']:.1f} kVA) already exceeds TR rating ({tr_rated:.0f} kVA). HC = 0 kW.")
            ...
            upsert_fleet_result(pea_id, tr_rated, None, None, hc_adj, status, verified=bool(matched_by_single))

            st.markdown(f"#### `{pea_id}` · {tr_rated:.0f} kVA · {day_type}")

            ca, cb, cc, cd = st.columns(4)
            CARD_CLASS = {
                "Chargers — daily": "card-green",
                "Chargers — simultaneous": "card-red",
            }
            for col, label, val, unit, sub in [
                (ca, "Predicted HC", f"{hc_adj:.1f}", "kW", "daily, time-distributed (DNOA)"),
                (cb, "Chargers — daily", f"{n_cars_daily}", "units", "spread across charging windows"),
                (cc, "Chargers — simultaneous", f"{n_cars_simultaneous}", "units", "drawing power at the same instant"),
                (cd, "Capacity headroom", f"{feats['Capacity_Headroom_kVA']:.1f}", "kVA", "TR rating − peak S_TOT"),
            ]:
                with col:
                    card_cls = CARD_CLASS.get(label, "card")
            ...
            with p1:
                st.markdown('<div class="section-lbl">Daily load profile</div>', unsafe_allow_html=True)
            ...
            with p2:
                st.markdown('<div class="section-lbl">HC by charging scenario</div>', unsafe_allow_html=True)
                fig2 = plot_scenario_bar(feats, tr_rated, user_weights, model, xgb_residual, scaler)
                st.pyplot(fig2, use_container_width=True)
                plt.close(fig2)

            st.markdown("---")
            st.markdown('<div class="section-lbl">Residential load characteristics</div>', unsafe_allow_html=True)
            ...
            with st.expander("🔧 Debug: Monte Carlo ground truth for this transformer", expanded=False):
                ...
                if run_debug:
                    ...
                    with dc1:
                        st.markdown(f'<div class="card"><div class="card-label">Monte Carlo HC (5th pct)</div><div class="card-value">{mc_result["hc_p5"]:.1f}<span class="card-unit">kW</span></div></div>', unsafe_allow_html=True)
                    with dc2:
                        st.markdown(f'<div class="card"><div class="card-label">Hybrid Engine (raw)</div><div class="card-value">{hc_raw:.1f}<span class="card-unit">kW</span></div></div>', unsafe_allow_html=True)
                    with dc3:
                        delta = hc_raw - mc_result['hc_p5']
                        st.markdown(f'<div class="card"><div class="card-label">Difference</div><div class="card-value">{delta:+.1f}<span class="card-unit">kW</span></div></div>', unsafe_allow_html=True)
...
with tab_fleet:
    ...
    fleet_valid = fleet_edit.dropna(subset=["Latitude", "Longitude"]).copy()
    ...
    if not fleet_valid.empty:
        ...
        st.markdown('<div class="section-lbl">Fleet map</div>', unsafe_allow_html=True)

        def compute_center_zoom(lat_tuple, lon_tuple, padding_factor=1.6):
            import math
            if not lat_tuple or not lon_tuple:
                return CHONBURI_LAT, CHONBURI_LON, 10.0
        ...
        st.markdown('<div class="section-lbl">Jump to analysis</div>', unsafe_allow_html=True)
        pick_col, btn_col = st.columns([3, 1])
        with pick_col:
            pick_name = st.selectbox("Select a transformer to analyze", fleet_valid["Name"].tolist(), label_visibility="collapsed")
        with btn_col:
            if st.button("Analyze →", width='stretch'):
                row = fleet_valid[fleet_valid["Name"] == pick_name].iloc[0]
                st.session_state.jump_to_name = pick_name
                if pd.notna(row["Rated_kVA"]):
                    set_tr_rated(float(row["Rated_kVA"]))
                st.rerun()
