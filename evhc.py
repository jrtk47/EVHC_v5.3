...
TRANSLATIONS = {
    ...
    "shift_utc":         {"EN": "UTC+7", "TH": "UTC+7"},
    "shift_utc_hint":    {"EN": "Timestamps are assumed UTC and shifted to UTC+7 by default. Toggle per-transformer in the Fleet table if a file is already in local time.",
                           "TH": "ค่าเวลาในไฟล์จะถูกสมมติเป็น UTC และแปลงเป็น UTC+7 โดยอัตโนมัติ หากไฟล์ใดเป็นเวลาท้องถิ่นอยู่แล้ว ให้ปิดสวิตช์นี้ในตารางหม้อแปลงเป็นรายตัว"},
    "avg_charger_caption": {"EN": "Average charger power from this mix: **{val:.2f} kW**", "TH": "กำลังไฟเฉลี่ยของเครื่องชาร์จจากสัดส่วนนี้: **{val:.2f} kW**"},
...
PEAK_HOURS = {
    ...
    "22:00  Late night (Late Charging)":     22.0,
}
...
def set_tr_rated(value: float):
    ...
    # Also sync the selectbox's own widget state — st.selectbox with an
    # explicit key ignores `index=` after its first render, so updating
    # tr_rated_input alone is not enough to move the widget. Snap to the
    # closest standard rating so the key always holds a valid option.
    closest = min(STANDARD_KVA, key=lambda k: abs(k - value))
    st.session_state["_tr_rated_selectbox"] = closest


def init_fleet():
    if "fleet" not in st.session_state:
        st.session_state.fleet = pd.DataFrame(
            columns=["Name", "Latitude", "Longitude", "Rated_kVA", "HC_kW", "Status", "Rated_Verified", "Shift_UTC"]
        ).astype({
            "Name": "object", "Latitude": "float64", "Longitude": "float64",
            "Rated_kVA": "float64", "HC_kW": "float64", "Status": "object",
            "Rated_Verified": "bool", "Shift_UTC": "bool",
        })
...
def upsert_fleet_result(name, tr_rated, lat, lon, hc_adj, status, verified=False, shift_utc=None):
    fleet = st.session_state.fleet
    mask = fleet["Name"] == name
    if mask.any():
        prev_status = fleet.loc[mask, "Status"].iloc[0] if mask.any() else None
        prev_hc = fleet.loc[mask, "HC_kW"].iloc[0] if mask.any() else None
        ...
        if shift_utc is not None and fleet.loc[mask, "Shift_UTC"].isna().all():
            fleet.loc[mask, "Shift_UTC"] = bool(shift_utc)
        # st.data_editor is keyed and caches its own snapshot on first render;
        # simply overwriting st.session_state.fleet does NOT make an already-
        # rendered data_editor pick up the change (same class of bug as the
        # kVA selectbox). Bump a generation counter so the editor's key
        # changes and it re-seeds from the fresh data whenever a value we
        # just wrote actually differs from what was there before.
        if prev_status != status or (prev_hc is not None and round(float(prev_hc), 1) != round(hc_adj, 1)):
            st.session_state["_fleet_editor_gen"] = st.session_state.get("_fleet_editor_gen", 0) + 1
    else:
        new_row = pd.DataFrame([{
            ...
            "Shift_UTC": bool(shift_utc) if shift_utc is not None else True,
        }])
...
""", unsafe_allow_html=True)
    ...
    if st.button(t("tab_fleet", lang), width='stretch',
        ...
        st.session_state.page = "fleet"
        # NOTE: deliberately NOT calling st.rerun() here. This button sits
        # BEFORE the sidebar config widgets (peak hour, kVA, day type,
        # charger mix) later in the script. An explicit st.rerun() at this
        # point interrupts the script before those widgets are reached,
        # which causes Streamlit to lose their session state on that partial
        # pass — this was the actual root cause of peak-hour/kVA/status
        # appearing to "reset" when switching tabs. The button click already
        # triggers Streamlit's normal automatic rerun; explicitly forcing
        # one early is redundant AND harmful.
    ...
    with st.popover("💾 Save / Load Fleet", use_container_width=True):
        ...
        if imported_zip is not None and st.session_state.get("_last_imported_name") != imported_zip.name:
            ...
            # NOTE: no explicit st.rerun() — see note on the Fleet/Analysis nav
            # buttons above. This popover is also positioned before the config
            # widgets later in the script; the file_uploader interaction already
            # triggers Streamlit's normal automatic rerun.
        ...
        with st.popover("📍 Set coordinates", use_container_width=True):
            ...
            with st.form("_coord_form", border=False):
                ...
                if st.form_submit_button("Apply", width='stretch'):
                    ...
                    # NOTE: no explicit st.rerun() here either — same reasoning
                    # as the nav buttons above. st.form_submit_button already
                    # triggers Streamlit's normal automatic rerun.
    ...
    if st.button(t("tab_analysis", lang), width='stretch',
        ...
        # See note on the Fleet button above — no explicit st.rerun() here either.
...
if _early_inspect_pick in _cached_names_early and st.session_state.get("_last_inspect_pick") != _early_inspect_pick:
    ...
    if not _row.empty and pd.notna(_row.iloc[0]["Rated_kVA"]) and bool(_row.iloc[0].get("Rated_Verified", False)):
        set_tr_rated(float(_row.iloc[0]["Rated_kVA"]))
...
with st.sidebar:
    with st.expander(t("cfg_header", lang), expanded=False):
        ...
        if st.session_state.show_explanations:
            st.caption(t("shift_utc_hint", lang))
        ...
        if "_tr_rated_selectbox" not in st.session_state:
            st.session_state["_tr_rated_selectbox"] = STANDARD_KVA[closest_idx]

        tr_rated = st.selectbox(
            t("nameplate_label", lang),
            STANDARD_KVA, key="_tr_rated_selectbox",
        ...
        day_type = st.radio(
            ...
            horizontal=True, key="_day_type_radio",
        )
        is_weekend = 1 if "Weekend" in day_type else 0

        st.markdown(f'<div class="sb-lbl">{t("sb5_title", lang)}</div>', unsafe_allow_html=True)
        peak_label = st.selectbox(t("peak_hour_label", lang), list(PEAK_HOURS.keys()), index=2, key="_peak_hour_selectbox")
        peak_hour  = PEAK_HOURS[peak_label]

        st.markdown(f'<div class="sb-lbl">{t("sb6_title", lang)}</div>', unsafe_allow_html=True)
        w_slow = st.slider("3.7 kW slow charger (%)", min_value=0, max_value=100, value=10, step=5, key="_w_slow_slider")
        w_std  = st.slider("7.4 kW standard charger (%)", min_value=0, max_value=max(0, 100 - w_slow), value=min(70, 100 - w_slow), step=5, key="_w_std_slider")
        ...
        user_avg_kw  = sum(c * w for c, w in zip(CHARGER_OPTIONS_KW, user_weights))
        st.caption(t("avg_charger_caption", lang, val=user_avg_kw))

if uploaded is not None:
    active_bytes = uploaded.getvalue()
    active_id = uploaded.name.replace('.csv', '')
    _prev_shift = st.session_state.fleet_files.get(active_id, (None, True))[1]
    st.session_state.fleet_files[active_id] = (active_bytes, _prev_shift)
    st.session_state.jump_to_name = active_id
    st.session_state._persistent_active = active_id # <--- ADD THIS LINE
    st.session_state._uploader_gen += 1
    st.rerun()

if batch_files:
    batch_signature = tuple(sorted(f.name for f in batch_files)) + (st.session_state.get("_ratings_version", 0),)
    if st.session_state.get("_last_batch_sig") != batch_signature:
        st.session_state._last_batch_sig = batch_signature
        batch_fids = []
        batch_meta = {}  # fid -> {"shape": str} or {"error": str} — one-time info that doesn't depend on peak hour / day type / charger mix
        prog = st.progress(0.0, text="Processing batch...")
        for i, f in enumerate(batch_files):
            fid = f.name.replace('.csv', '')
            fbytes = f.getvalue()
            file_shift_utc = st.session_state.fleet_files.get(fid, (None, True))[1]
            try:
                ...
                if {'timestamp', 'P_TOT', 'S_TOT'} - _probe_cols:
                    batch_meta[fid] = {"error": "Missing columns"}
                else:
                    dfc = clean_and_resample(fbytes, file_shift_utc)
                    ...
                    upsert_fleet_result(fid, rated_b, lat_b, lon_b, hc_adj_b, status_b, verified=bool(matched_by_b), shift_utc=file_shift_utc)
                    st.session_state.fleet_files[fid] = (fbytes, file_shift_utc)
                    batch_fids.append(fid)
                    batch_meta[fid] = {"shape": shape_flag_b}
            except Exception as e:
                batch_meta[fid] = {"error": f"Error: {e}"}
            prog.progress((i + 1) / len(batch_files), text=f"Processed {i + 1}/{len(batch_files)}")
        prog.empty()
        st.session_state._batch_fids = list(batch_meta.keys())
        st.session_state._batch_meta = batch_meta
        st.session_state._uploader_gen += 1
        st.rerun()

# ══════════════════════════════════════════════════════════════
# FLEET SYNCHRONIZATION (Auto-Recalculate on Config Change)
# ══════════════════════════════════════════════════════════════

def _recompute_all_fleet_entries(is_weekend, peak_hour, user_weights, user_avg_kw, tr_rated):
    """Recompute HC/Status for every uploaded transformer using the given
    config, and upsert results into st.session_state.fleet. Pulled out into
    a function so it can be called both from the background config-change
    sync below AND unconditionally right before the Fleet page renders —
    the latter guarantees the Fleet tab is never stale after a tab switch,
    regardless of whether the background sync's change-detection fired."""
    for fid, (fbytes, file_shift_utc) in st.session_state.fleet_files.items():
        try:
            dfc = clean_and_resample(fbytes, file_shift_utc)
            dfc['_dow'] = dfc.index.dayofweek
            dday = dfc[dfc['_dow'] >= 5 if is_weekend else dfc['_dow'] < 5].drop(columns='_dow')
            ...
            if user_avg_kw <= 0 or feats_b['Capacity_Headroom_kVA'] <= 0:
                ...
                hc_raw_b, hc_adj_b = predict_and_adjust(
                    feats_b, peak_hour, user_weights, rated_b,
                    model, xgb_residual, scaler
                )
            ...
            upsert_fleet_result(
                fid, rated_b, lat_b, lon_b, hc_adj_b, status_b,
                verified=bool(matched_by_b), shift_utc=file_shift_utc
            )
        except Exception:
            # Swallow errors here — a bad file surfaces on the Analysis tab itself.
            pass


# สร้าง Signature เพื่อใช้เช็คว่ามีการปรับเปลี่ยนค่าใน Sidebar หรือไม่
current_config_sig = (is_weekend, peak_hour, tuple(user_weights), tr_rated)

if st.session_state.get("_last_global_config") != current_config_sig:
    # เริ่ม Sync ก็ต่อเมื่อไม่ใช่การโหลดแอปครั้งแรก และมีข้อมูลหม้อแปลงอยู่ในระบบแล้ว
    if st.session_state.get("_last_global_config") is not None and st.session_state.get("fleet_files"):
        _recompute_all_fleet_entries(is_weekend, peak_hour, user_weights, user_avg_kw, tr_rated)
        # Only mark this config as "synced" if we actually had a chance to
        # recompute. Previously this was set unconditionally, so a skipped
        # recompute (e.g. fleet_files empty at that moment) would silently
        # mark a stale config as up to date and the real sync would never
        # be retried until the sig happened to change again.
        st.session_state._last_global_config = current_config_sig
    else:
        st.session_state._last_global_config = current_config_sig
...
# ══════════════════════════════════════════════════════════════
# ACTIVE TRANSFORMER — resolved & recomputed unconditionally (every
# rerun, on any page) so the Fleet map/status stays in sync the
# instant a sidebar param (peak hour, day type, charger mix, rating,
# ...) changes — not just when the Analysis page happens to be open.
# ══════════════════════════════════════════════════════════════

cached_names = sorted(st.session_state.fleet_files.keys())

# 1. Sync persistent tracker with selectbox if it is currently rendered and valid
if st.session_state.get("_inspect_pick") in cached_names:
    st.session_state._persistent_active = st.session_state["_inspect_pick"]

# 2. Robust fallback: If tracker is missing, None, or out-of-bounds (e.g. after fresh batch/zip import), force a valid fallback
if st.session_state.get("_persistent_active") not in cached_names:
    st.session_state._persistent_active = prefill if prefill in cached_names else (cached_names[0] if cached_names else None)
    
_active_pick = st.session_state._persistent_active

active_bytes = None
active_id = None
active_shift_utc = True

if uploaded is not None:
    active_bytes = uploaded.getvalue()
    active_id = uploaded.name.replace('.csv', '')
    active_shift_utc = st.session_state.fleet_files.get(active_id, (None, True))[1]
elif _active_pick is not None and _active_pick in st.session_state.fleet_files:
    active_bytes, active_shift_utc = st.session_state.fleet_files[_active_pick]
    active_id = _active_pick

analysis_error = None
analysis_missing_cols = None
feats = None
shape_check = None
hc_raw = hc_adj = None
status = badge_cls = None
pea_id = active_id
...
if active_bytes is not None:
    try:
        ...
        _probe_cols = set(_probe.columns) | set(_normalize_dtms_columns(_probe.columns).values())
        analysis_missing_cols = {'timestamp', 'P_TOT', 'S_TOT'} - _probe_cols
    except Exception as e:
        analysis_error = t("file_read_error", lang, e=e)

    if not analysis_missing_cols and analysis_error is None:
        plt.close('all')  # guard against stale figures lingering from a prior errored/partial run
        df_day = clean_and_resample(active_bytes, active_shift_utc)
        df_day['_dow'] = df_day.index.dayofweek
        df_day = df_day[df_day['_dow'] >= 5 if is_weekend else df_day['_dow'] < 5].drop(columns='_dow')
        if df_day.empty:
            df_day = clean_and_resample(active_bytes, active_shift_utc).drop(columns='_dow')

        # 1. Look up Master File rating BEFORE extracting features
        actual_rated, matched_by_single = lookup_rated_kva(pea_id, tr_rated)

        # 2. Use actual_rated instead of the sidebar's tr_rated fallback
        feats = extract_features(df_day, actual_rated, is_weekend)
        shape_check = check_residential_shape(df_day, feats)

        if feats['Capacity_Headroom_kVA'] <= 0:
            hc_raw, hc_adj = 0.0, 0.0
        else:
            hc_raw, hc_adj = predict_and_adjust(feats, peak_hour, user_weights, actual_rated, model, xgb_residual, scaler)
        ...
        # 3. Upsert using actual_rated to prevent the sidebar from permanently corrupting the fleet table
        upsert_fleet_result(pea_id, actual_rated, lat_single, lon_single, hc_adj, status, verified=bool(matched_by_single), shift_utc=active_shift_utc)

# ══════════════════════════════════════════════════════════════
# PAGE — TRANSFORMER ANALYSIS
# ══════════════════════════════════════════════════════════════
if st.session_state.page == "analysis":
    if st.session_state.get("_batch_fids"):
        _batch_meta = st.session_state.get("_batch_meta", {})
        _fleet_by_name = st.session_state.fleet.set_index("Name") if not st.session_state.fleet.empty else None
        _batch_rows = []
        for fid in st.session_state._batch_fids:
            meta = _batch_meta.get(fid, {})
            if "error" in meta:
                _batch_rows.append((fid, None, None, None, meta["error"], "—"))
                continue
            if _fleet_by_name is not None and fid in _fleet_by_name.index:
                frow = _fleet_by_name.loc[fid]
                matched_by = "master file" if bool(frow.get("Rated_Verified", False)) else "sidebar fallback"
                _batch_rows.append((
                    fid,
                    frow.get("Rated_kVA"),
                    matched_by,
                    round(float(frow["HC_kW"]), 1) if pd.notna(frow.get("HC_kW")) else None,
                    frow.get("Status"),
                    meta.get("shape", "—"),
                ))
            else:
                _batch_rows.append((fid, None, None, None, "Not found", meta.get("shape", "—")))

        with st.expander(f"📦 Batch results ({len(_batch_rows)} transformers)", expanded=False):
            st.dataframe(
                pd.DataFrame(_batch_rows, columns=["Transformer", "Rated (kVA)", "Matched by", "HC (kW)", "Status", "Shape"]),
    ...
    inspect_pick = None
    if cached_names:
        # Sync the selectbox index with the persistent state to avoid front-to-back visual mismatch
        if st.session_state.get("_persistent_active") in cached_names:
            default_idx = cached_names.index(st.session_state._persistent_active)
        else:
        ...
        if uploaded is None and inspect_pick is not None:
    ...
    if active_bytes is None:
    ...
    elif analysis_missing_cols:
        st.error(t("missing_cols", lang, missing=analysis_missing_cols))
    else:
        ...
        if feats['Capacity_Headroom_kVA'] <= 0:
            st.warning(t("exceeds_rating", lang, peak=feats['_peak_kva'], rated=actual_rated))
        ...
        st.markdown(f"#### `{pea_id}` · {actual_rated:.0f} kVA · {day_type_display}")
        ...
        with p1:
            ...
            try:
                fig1 = plot_load_curve(
                    feats['_profile_kva'], feats['_profile_kw'], actual_rated, hc_adj, peak_hour,
                ...
                st.error(f"Couldn't render daily profile chart: {e}")

        with p2:
            st.markdown(f'<div class="section-lbl">{t("section_hc_scenario", lang)}</div>', unsafe_allow_html=True)
            try:
                fig2 = plot_scenario_bar(feats, actual_rated, user_weights, model, xgb_residual, scaler)
                st.pyplot(fig2, use_container_width=True)
                plt.close(fig2)
            except Exception as e:
                st.error(f"Couldn't render scenario chart: {e}")
        ...
        try:
            ...
            st.error(f"Couldn't render residential profile chart: {e}")
        ...
        with st.expander("🔧 Debug: Monte Carlo ground truth for this transformer", expanded=False):
            with st.form(key="_mc_debug_form"):
                n_iter = st.slider("Monte Carlo iterations", min_value=50, max_value=1000, value=200, step=50)
                run_debug = st.form_submit_button("▶ Run live Monte Carlo for this transformer")

            if run_debug:
                try:
                    with st.spinner(f"Running {n_iter} Monte Carlo iterations..."):
                        np.random.seed(42)
                        mc_result = run_scenario_monte_carlo(feats['_profile_kva'], actual_rated, user_weights, peak_hour, n_iter=n_iter)
                    st.session_state["_mc_result"] = mc_result
                    st.session_state["_mc_result_id"] = pea_id
                    st.session_state["_mc_result_hc_raw"] = hc_raw
                    st.session_state["_mc_result_profile_kva"] = feats['_profile_kva']
                    st.session_state["_mc_result_tr_rated"] = actual_rated
                except Exception as e:
                    st.error(f"Monte Carlo simulation failed: {e}")
                    st.session_state["_mc_result"] = None

            if st.session_state.get("_mc_result_id") == pea_id and st.session_state.get("_mc_result") is not None:
                mc_result = st.session_state["_mc_result"]
                mc_hc_raw = st.session_state["_mc_result_hc_raw"]
                mc_profile_kva = st.session_state["_mc_result_profile_kva"]
                mc_tr_rated = st.session_state["_mc_result_tr_rated"]
                ...
                with dc2:
                    st.markdown(f'<div class="card"><div class="card-label">{t("mc_card_hybrid_raw", lang)}</div><div class="card-value">{mc_hc_raw:.1f}<span class="card-unit">kW</span></div></div>', unsafe_allow_html=True)
                with dc3:
                    delta = mc_hc_raw - mc_result['hc_p5']
                ...
                try:
                    ...
                    ax_mc.axvline(mc_hc_raw, color=AMBER, lw=1.5, linestyle='--', label=f'Hybrid raw = {mc_hc_raw:.1f} kW')
                    ...
                    st.error(f"Couldn't render Monte Carlo histogram: {e}")
                ...
                with mc1:
                    try:
                        ...
                        st.error(f"Couldn't render accumulation chart: {e}")

                with mc2:
                    try:
                        rep_ev_profile = mc_result['rep_ev_profile']
                        x = np.arange(96)
                        combined = mc_profile_kva + rep_ev_profile
                        fig_load, ax_load = plt.subplots(figsize=(6, 3.0))
                        fig_load.patch.set_facecolor('white')
                        ax_load.fill_between(x, mc_profile_kva, color=BLUE, alpha=0.15, zorder=1)
                        ax_load.plot(x, mc_profile_kva, color=BLUE, lw=1.5, label='Base load (S_TOT)', zorder=2)
                        ax_load.fill_between(x, mc_profile_kva, combined, color=GREEN, alpha=0.30, zorder=1, label='EV load')
                        ax_load.plot(x, combined, color=GREEN, lw=1.5, zorder=3)
                        ax_load.axhline(mc_tr_rated, color=RED, lw=1.5, linestyle='--', label=f'TR Rating ({mc_tr_rated:.0f} kVA)', zorder=4)
                        ...
                        st.error(f"Couldn't render combined load chart: {e}")

# ══════════════════════════════════════════════════════════════
# FLEET MAP — animated Leaflet card (status legend fused into map)
# ══════════════════════════════════════════════════════════════
def render_fleet_map(fleet_valid, lang):
    import math
    ...
    def _darken(hexcolor, amt=0.35):
        r, g, b = _hex_to_rgb(hexcolor)
        return _rgb_to_hex((r * (1 - amt), g * (1 - amt), b * (1 - amt)))

    def compute_center_zoom(lat_tuple, lon_tuple, padding_factor=1.25, zoom_boost=1.0):
    ...
    n_ready      = int((fleet_valid["Status"] == "Ready").sum())
    n_monitor    = int((fleet_valid["Status"] == "Monitor").sum())
    n_critical   = int((fleet_valid["Status"] == "Critical").sum())
    n_unassessed = int((fleet_valid["Status"] == "Unassessed").sum())

    markers = []
    for _, r in fleet_valid.iterrows():
        status = r["Status"] if pd.notna(r["Status"]) else "Unassessed"
        ...
        markers.append({
            ...
            "status": status,
            "color": c,
            "colorLight": _lighten(c),
            "colorDark": _darken(c),
            "kva": (f'{r["Rated_kVA"]:.0f}' if pd.notna(r.get("Rated_kVA")) else "—"),
            "hc": (r["Predicted EV HC"] if pd.notna(r.get("HC_kW")) else "not assessed"),
        })

    labels = {
        "Ready": "Ready" if lang == "EN" else "พร้อม",
        "Monitor": "Monitor" if lang == "EN" else "เฝ้าระวัง",
        "Critical": "Critical" if lang == "EN" else "วิกฤต",
        "Unassessed": "Unassessed" if lang == "EN" else "ยังไม่ประเมิน",
    }
    ...
    payload = json.dumps({
        ...
        "zoom": auto_zoom,
        "counts": {"Ready": n_ready, "Monitor": n_monitor, "Critical": n_critical, "Unassessed": n_unassessed},
        "colors": color_map,
        "labels": labels,
    })
...
<style>
  html, body { margin:0; padding:0; height:100%; font-family:'IBM Plex Sans Thai', sans-serif; }
  #card {
    position:relative; width:100%; height:__MAP_HEIGHT__px;
    border-radius:14px; overflow:hidden; border:1px solid __BORDER__;
  ...
  #legend {
    ...
    padding:10px 14px; box-shadow:0 4px 14px rgba(15,23,42,0.12);
    display:flex; gap:16px; align-items:center;
    animation: fadeSlideIn 0.5s ease-out;
  }
  .leg-item { display:flex; flex-direction:column; align-items:center; min-width:44px; }
  .leg-dot { width:9px; height:9px; border-radius:50%; margin-bottom:3px; box-shadow:0 0 6px currentColor; }
  .leg-n { font-size:16px; font-weight:700; line-height:1; color:__SLATE__; }
  .leg-l { font-size:9px; text-transform:uppercase; letter-spacing:0.05em; color:__SLATE_MID__; margin-top:2px; }

  #titlebar {
    position:absolute; bottom:14px; left:14px; z-index:1000;
    background:rgba(30,41,59,0.85); backdrop-filter:blur(6px);
    color:#f8fafc; font-size:11px; font-family:'IBM Plex Mono',monospace;
    padding:6px 12px; border-radius:8px; letter-spacing:0.03em;
  ...
  /* Leaflet zoom control + custom home/reset control, styled to match card chrome */
  ...
  .marker-ring.active { animation: ringPulse 1.8s ease-out infinite; }
  .marker-ring.active.r2 { animation-delay:0.9s; }
  .marker-dot {
    position:absolute; top:50%; left:50%; width:13px; height:13px;
    margin:-6.5px 0 0 -6.5px; border-radius:50%;
    background:radial-gradient(circle at 35% 30%, var(--cl) 0%, var(--c) 58%, var(--cd) 100%);
    border:1.5px solid rgba(255,255,255,0.9);
    box-shadow:0 1px 2px rgba(15,23,42,0.35), inset 0 -2px 2px rgba(0,0,0,0.18), inset 0 1px 1px rgba(255,255,255,0.4);
    transition:transform 0.15s ease, box-shadow 0.15s ease;
    cursor:pointer;
  }
  .marker-wrap:hover .marker-dot {
    transform:scale(1.18);
    box-shadow:0 2px 6px rgba(15,23,42,0.4), inset 0 -2px 2px rgba(0,0,0,0.18), inset 0 1px 1px rgba(255,255,255,0.4);
  }
  ...
  .leaflet-popup-content { font-family:'IBM Plex Sans Thai', sans-serif; font-size:12.5px; margin:10px 12px; }
  .pop-title { font-weight:700; color:__SLATE__; margin-bottom:4px; font-size:13px; }
  .pop-row { display:flex; justify-content:space-between; gap:14px; color:__SLATE_MID__; padding:1px 0; }
  .pop-badge { display:inline-block; padding:1px 8px; border-radius:20px; font-size:10.5px; font-weight:600; color:white; margin-top:5px; }
</style>
</head>
<body>
<div id="card">
  <div id="map"></div>
  <div id="legend"></div>
  <div id="titlebar">🗺️ FLEET MAP &nbsp;·&nbsp; CHONBURI GRID</div>
</div>
<script>
const DATA = __PAYLOAD__;

const map = L.map('map', { zoomControl:true, scrollWheelZoom:true, fadeAnimation:true, zoomAnimation:true, markerZoomAnimation:true });
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  attribution:'&copy; OpenStreetMap contributors', maxZoom:19,
}).addTo(map);

// Start zoomed-out slightly wider, then fly in for a smooth intro animation
map.setView(DATA.center, Math.max(DATA.zoom - 2.2, 3));
setTimeout(() => {
  map.flyTo(DATA.center, DATA.zoom, { duration: 1.3, easeLinearity: 0.22 });
}, 200);

// Reset-to-home control, stacked under the default zoom control (top-left)
const HomeControl = L.Control.extend({
  options: { position: 'topleft' },
  onAdd: function() {
    const div = L.DomUtil.create('div', 'leaflet-bar leaflet-control-home');
    div.innerHTML = '<a href="#" title="Reset view" role="button">⌂</a>';
    L.DomEvent.disableClickPropagation(div);
    L.DomEvent.on(div, 'click', (e) => {
      e.preventDefault();
      map.flyTo(DATA.center, DATA.zoom, { duration: 0.9, easeLinearity: 0.22 });
    });
    return div;
  },
});
...
function clearSelection() {
  if (selectedWrap) {
    ...
    selectedWrap.querySelectorAll('.marker-ring').forEach(r => r.classList.remove('active'));
  }
  selectedWrap = null;
}
...
DATA.markers.forEach(m => {
  const icon = L.divIcon({
    className: '',
    html: `<div class="marker-wrap" style="--c:${m.color};--cl:${m.colorLight};--cd:${m.colorDark}">
             <div class="marker-ring r1"></div>
             <div class="marker-ring r2"></div>
  ...
  marker.bindPopup(
    `<div class="pop-title">${m.name}</div>
     <div class="pop-row"><span>Rated</span><span>${m.kva} kVA</span></div>
     <div class="pop-row"><span>Predicted EV HC</span><span>${m.hc}</span></div>
     <span class="pop-badge" style="background:${m.color}">${m.status}</span>`
  );
  marker.on('click', () => {
    const wrap = marker.getElement() ? marker.getElement().querySelector('.marker-wrap') : null;
    if (!wrap) return;
    if (selectedWrap && selectedWrap !== wrap) clearSelection();
    wrap.classList.add('selected');
    wrap.querySelectorAll('.marker-ring').forEach(r => r.classList.add('active'));
    selectedWrap = wrap;
  });
  marker.on('popupclose', () => { if (selectedWrap && selectedWrap === (marker.getElement() && marker.getElement().querySelector('.marker-wrap'))) clearSelection(); });
});

map.on('click', () => clearSelection());

// Fused status legend
const order = ["Ready", "Monitor", "Critical", "Unassessed"];
const legendEl = document.getElementById('legend');
legendEl.innerHTML = order.map(k => `
  <div class="leg-item">
    <div class="leg-dot" style="background:${DATA.colors[k]}; color:${DATA.colors[k]};"></div>
    <div class="leg-n" style="color:${DATA.colors[k]}">${DATA.counts[k]}</div>
    <div class="leg-l">${DATA.labels[k]}</div>
  </div>
`).join('');
</script>
</body>
...
"""
    html = (html
            ...
            .replace("__MAP_HEIGHT__", str(MAP_HEIGHT_PX))
            .replace("__BORDER__", BORDER)
            .replace("__SLATE__", SLATE)
            .replace("__SLATE_MID__", SLATE_MID))
    components.html(html, height=MAP_HEIGHT_PX + 8, scrolling=False)
...
# PAGE — FLEET OVERVIEW
# ══════════════════════════════════════════════════════════════
def render_empty_map(lang):
    prompt = t("empty_state", lang)
    html = """
<!DOCTYPE html>
<html>
<head>
...
<style>
  ...
  #map { position:absolute; inset:0; filter:saturate(0.55) brightness(1.03); }
  #veil { position:absolute; inset:0; background:rgba(248,250,252,0.55); z-index:600; pointer-events:none; }
  ...
  .prompt-ring {
    ...
    background:__BLUE__; opacity:0.35;
    animation: bigPulse 2.4s ease-out infinite;
  }
  .prompt-ring.r2 { animation-delay:1.2s; }
  .prompt-icon {
    position:absolute; inset:0; display:flex; align-items:center; justify-content:center;
    border-radius:50%; background:white; border:1.5px solid rgba(226,232,240,0.9);
    box-shadow:0 4px 14px rgba(15,23,42,0.15);
    font-size:26px;
  }
  .prompt-title {
    font-size:15px; font-weight:600; color:__SLATE__;
    background:rgba(255,255,255,0.9); backdrop-filter:blur(6px);
    padding:8px 18px; border-radius:20px; border:1px solid rgba(226,232,240,0.9);
...
<div id="card">
  ...
  <div id="veil"></div>
  <div id="prompt">
    <div class="prompt-pulse">
      <div class="prompt-ring"></div>
      <div class="prompt-ring r2"></div>
      <div class="prompt-icon">📂</div>
    </div>
    <div class="prompt-title">__PROMPT__</div>
  </div>
</div>
<script>
const map = L.map('map', { zoomControl:false, scrollWheelZoom:false, dragging:false, doubleClickZoom:false, boxZoom:false, keyboard:false, attributionControl:false, touchZoom:false });
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom:19 }).addTo(map);
map.setView([__CENTER_LAT__, __CENTER_LON__], 10);
</script>
</body>
</html>
"""
    html = (html
            ...
            .replace("__CENTER_LAT__", str(CHONBURI_LAT))
            .replace("__CENTER_LON__", str(CHONBURI_LON)))

    components.html(html, height=MAP_HEIGHT_PX + 8, scrolling=False)


if st.session_state.page == "fleet":
    # Belt-and-braces: recompute every transformer against the CURRENT
    # sidebar config right before rendering, regardless of whether the
    # background sync above already caught this change. This guarantees
    # the Fleet map/table are never one step behind after switching tabs.
    _fleet_render_sig = (is_weekend, peak_hour, tuple(user_weights), tr_rated)
    if st.session_state.get("_last_fleet_render_sig") != _fleet_render_sig and st.session_state.get("fleet_files"):
        _recompute_all_fleet_entries(is_weekend, peak_hour, user_weights, user_avg_kw, tr_rated)
    st.session_state["_last_fleet_render_sig"] = _fleet_render_sig
    ...
    if not fleet_valid.empty:
        ...
        render_fleet_map(fleet_valid, lang)
    else:
        render_empty_map(lang)

    with st.expander(t("section_fleet_table", lang), expanded=False):
        if st.session_state.show_explanations:
            st.caption(t("shift_utc_hint", lang))
        st.session_state.setdefault("_fleet_editor_gen", 0)
        fleet_edit = st.data_editor(
            ...
            column_config={
                ...
                "Shift_UTC": st.column_config.CheckboxColumn(t("shift_utc", lang), default=True),
                "Rated_Verified": None,
            },
            key=f"fleet_editor_{st.session_state['_fleet_editor_gen']}",
        )

        if not fleet_edit.equals(st.session_state.fleet):
            # Propagate any Shift_UTC toggles into the stored (bytes, shift_utc) pairs
            # used by clean_and_resample, so the next chart render actually reflects them.
            changed = fleet_edit.set_index("Name")["Shift_UTC"]
            prev = st.session_state.fleet.set_index("Name")["Shift_UTC"] if "Shift_UTC" in st.session_state.fleet.columns else None
            for name, new_val in changed.items():
                if pd.isna(new_val):
                    continue
                if name in st.session_state.fleet_files:
                    raw, _ = st.session_state.fleet_files[name]
                    st.session_state.fleet_files[name] = (raw, bool(new_val))
